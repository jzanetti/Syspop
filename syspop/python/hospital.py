...
def create_hospital_names(hospital_data: DataFrame) -> DataFrame:
    ...
    hospital_data["hospital_name"] = (
        hospital_data.groupby("area").cumcount().astype(str)
    )
    hospital_data["hospital_name"] = (
...
def hospital_wrapper(
    ...
    pop_data = pop_data.drop(
        columns=["latitude_hospital", "longitude_hospital", "beds", "area_hospital"]
    )
    ...
    second_nearest_rows = hospital_data.iloc[second_nearest_indices].reset_index(
        drop=True
    )
...
def get_hospital_address(pop_data: DataFrame, address_data: DataFrame) -> DataFrame:
    ...
    """
    unique_hospitals = pop_data[
        ["hospital_name", "latitude_hospital", "longitude_hospital", "area"]
    ].drop_duplicates()
    unique_hospitals = unique_hospitals.rename(
        columns={
            "hospital_name": "name",
